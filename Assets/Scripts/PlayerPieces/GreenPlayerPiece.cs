...
public class GreenPlayerPiece : PlayerPiece
{
    ...
    private void Start()
    {
        rollDice = GetComponentInParent<GreenHome>().rollingDice;
    }
        ...
        if (canMove == true)
        {
            if (numberOfStepsToMove > 0)
            {
                if (numberOfStepsAlreadyMoved + numberOfStepsToMove <= 33)
                {
                    ...
                    {
                        transform.position = pathPoints.greenPathPoints[i + 1].transform.position;
                        yield return new WaitForSeconds(0.25f);
                    }
    ...
    {
        isReady = true;
        transform.position = transform.position = pathPoints.greenPathPoints[0].transform.position;
